...
function triggercommandHandler(index) {
    ...
    switch (index) {
        case 0:
            script.commandHandler.nextCommand();
            break;
        case 1:
            script.commandHandler.previousCommand();
            break;
        case 2:
            script.commandHandler.leftCommand();
            break;        
        case 3:
            script.commandHandler.rightCommand();
            break;   
        case 4:
            script.commandHandler.upCommand();
            break; 
        case 5:
            script.commandHandler.downCommand();
            break;
        default:
            script.commandHandler.numberCommand(index-6);
    }
...
function initializeSpeechContexts() {
    ...
    for (var i = 0;i<scComponents.length;i++) {
        ...
        if (scComponent && scComponent.enabled) {
            var data = {
                phrases: scComponent.getContext!==null && scComponent.getContext(voiceMLLanguage),
                boostValue: scComponent.getBoostValue!==null && scComponent.getBoostValue(),  
            };
...
function initializeKeywords() {
    ...
    for (var i = 0; i < parent.getChildrenCount(); i++) {
        ...
        if (child.enabled) {
            ...
            var data = {
                keyword: st.getKeyword!==null && st.getKeyword()["Keyword"],
                categoryAliases: st.getCategoryAliases!==null && st.getKeyword()["CategoryAliases"],
                triggerBehaviors: st.getBehaviorComponents!==null && st.getBehaviorComponents(),
            };
